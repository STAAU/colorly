import SwiftUI

struct MainTabView: View {
    @Environment(AppModel.self) private var model

    var body: some View {
        @Bindable var model = model
        TabView(selection: $model.selectedTab) {
            HomeView().tag(0)
            ExploreView().tag(1)
            GalleryView().tag(2)
        }
        .toolbar(.hidden, for: .tabBar)
        .safeAreaInset(edge: .bottom, spacing: 0) {
            FloatingTabBar(selection: $model.selectedTab)
        }
    }
}

private struct FloatingTabBar: View {
    @Binding var selection: Int
    private let items = [("Home", "house.fill"), ("Explore", "sparkles.rectangle.stack.fill"), ("Gallery", "photo.stack.fill")]

    var body: some View {
        HStack(spacing: 0) {
            ForEach(items.indices, id: \.self) { index in
                Button { selection = index } label: {
                    VStack(spacing: 4) {
                        Image(systemName: items[index].1).font(.system(size: 19, weight: .semibold))
                        Text(items[index].0).font(.caption2.weight(selection == index ? .bold : .semibold))
                    }
                    .foregroundStyle(selection == index ? Color.artLavender : Color.artInk.opacity(0.45))
                    .frame(maxWidth: .infinity, minHeight: 50)
                    .contentShape(Rectangle())
                }
                .buttonStyle(PressScaleStyle())
                .accessibilityLabel(items[index].0)
                .accessibilityAddTraits(selection == index ? .isSelected : [])
            }
        }
        .padding(.top, 10)
        .padding(.bottom, 20)
        .background(alignment: .top) {
            Rectangle().fill(.primary.opacity(0.08)).frame(height: 1)
        }
        .background(.bar)
    }
}
